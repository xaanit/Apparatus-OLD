...
import static me.xaanit.apparatus.util.CuteUtil.GifType.HUG;
...
public class Hug implements ICommand {
    ...
    public void runCommand(IUser user, IChannel channel, IGuild guild, IMessage message, String[] args, IDiscordClient client) {
        ...
        em.withImage(CuteUtil.getRandomGif(HUG));
        em.withFooterText("Requested by: " + getNameAndDescrim(user));
        sendMessage(channel, user.mention() + " hugged " + u.mention(), em.build());
    }
}
