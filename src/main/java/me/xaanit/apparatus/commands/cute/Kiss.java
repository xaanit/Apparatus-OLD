...
import static me.xaanit.apparatus.util.CuteUtil.GifType.KISS;
...
public class Kiss implements ICommand {
    ...
    public void runCommand(IUser user, IChannel channel, IGuild guild, IMessage message, String[] args, IDiscordClient client) {
        ...
        em.withImage(CuteUtil.getRandomGif(KISS));
        em.withFooterText("Requested by: " + getNameAndDescrim(user));
        sendMessage(channel, user.mention() + " kissed " + u.mention(), em.build());
    }
}
