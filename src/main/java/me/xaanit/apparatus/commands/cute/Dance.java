...
import static me.xaanit.apparatus.util.CuteUtil.GifType.DANCING;
...
public class Dance implements ICommand {
    ...
    public void runCommand(IUser user, IChannel channel, IGuild guild, IMessage message, String[] args, IDiscordClient client) {
        ...
        em.withImage(CuteUtil.getRandomGif(DANCING));
        em.withFooterText("Requested by: " + getNameAndDescrim(user));
        sendMessage(channel, user.mention() + " danced with " + u.mention(), em.build());
    }
}
